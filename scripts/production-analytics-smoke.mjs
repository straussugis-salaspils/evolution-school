...
try {
  ...
  if (skipped === 0) {
    ...
    check("production shows the approved Google-only choice hierarchy", await value("(() => { const copy = document.querySelector('.eh-consent__copy')?.textContent || ''; const buttons = [...document.querySelectorAll('.eh-consent__button')]; const primary = getComputedStyle(buttons[0]); const secondary = getComputedStyle(buttons[1]); return copy.includes('Чтобы нужное находилось быстрее') && copy.includes('Google Analytics') && copy.includes('без рекламного отслеживания') && !copy.includes('Яндекс') && buttons[0]?.textContent.trim() === 'Разрешить' && buttons[1]?.textContent.trim() === 'Google без cookies' && primary.backgroundColor !== 'rgba(0, 0, 0, 0)' && secondary.backgroundColor === 'rgba(0, 0, 0, 0)'; })()"));
