...
try {
  ...
  for (const route of ROUTES) {
    ...
    const advancedConsentReady = await value(`${gaScript} === 1 && ${metrikaScript} === 0 && typeof window.gtag === 'function' && Array.isArray(window.dataLayer) && window.dataLayer?.[0]?.[2]?.analytics_storage === 'denied' && typeof window.ym === 'undefined'`);
    ...
    failures += ok ? 0 : 1;
    console.log(`${ok ? "PASS" : "FAIL"} ${route}: banner=${banner}, settings=${settings}, google-cookieless=${google().length}, yandex-before-consent=${yandex().length}, advanced-consent-ready=${advancedConsentReady}, analytics-cookies-absent=${noCookies}, overflow=${overflow}`);
  }
  if (skipped === 0) {
    urls.length = 0;
    ...
    check("production queues default Consent Mode v2 before the grant", await value("window.dataLayer?.[0]?.[0] === 'consent' && window.dataLayer?.[0]?.[1] === 'default' && Object.values(window.dataLayer?.[0]?.[2] || {}).length === 4 && Object.values(window.dataLayer?.[0]?.[2] || {}).every((state) => state === 'denied') && window.dataLayer.findIndex((item) => item?.[0] === 'consent' && item?.[1] === 'update' && item?.[2]?.analytics_storage === 'granted') > window.dataLayer.findIndex((item) => item?.[0] === 'config')"));
    ...
    await wait(850);
    check("production keeps Google cookieless and Yandex absent after revocation", await value(`${gaScript} === 1 && ${metrikaScript} === 0 && typeof window.gtag === 'function' && window.dataLayer?.[0]?.[2]?.analytics_storage === 'denied' && typeof window.ym === 'undefined'`));
