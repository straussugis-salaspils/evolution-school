...
const visualAlt = {
  ...
  R01: "Женщина с молодым ростком работает над глиняной формой в мастерской",
  R02: "Женщина выбирает цвета и фактуры, сверяясь со своим вкусом",
  R03: "Женщина с картой показывает конкретный маршрут и сохраняет границы",
  R04: "Женщина собирает ясную систему из нескольких решений",
  R05: "Женщина и мужчина обсуждают общий порядок за столом",
  ...
  R06: "Женщина удерживает спокойное внимание, пока семья занята своими делами",
  R07: "Наставница поддерживает молодую женщину, пока та сама ухаживает за растением",
  R08: "Женщина с картой выбирает путь на вечерней городской развилке",
...
const hubTeasers = {
  ...
  R01: "Персефона возвращает живое «хочу», которое не обязано сразу становиться бизнесом, пользой или новым пунктом плана.",
  R02: "Афродита начинается не с чужого взгляда, а с собственного вкуса: что оживляет, что не подходит и что вы выбираете сами.",
  R03: "Артемида не заставляет тащить больше: она помогает выбрать точный шаг, обозначить границу и действовать в своей мере.",
  ...
  R05: "Гера помогает выбрать, что мы строим, сделать невидимые договоры видимыми и перестать удерживать общую жизнь в одиночку.",
  R06: "Гестия показывает тишину не как побег от дел, а как место, где лишнее отпадает и главное получает форму.",
  R07: "Деметра создаёт тепло, в котором ребёнок, ученик, партнёр или проект могут расти, не превращая заботу в жизнь вместо другого.",
  R08: "Геката помогает завершить прежний этап, отличить ясную границу от ответного удара и дать следующему конкретную форму.",
...
<head>
  ...
  <meta property="article:modified_time" content="${article.modified_date || publishedDate}">
