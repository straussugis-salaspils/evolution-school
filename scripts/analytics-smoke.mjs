...
try {
  ...
  check("Google choice uses the approved copy and action order", await evaluate("(() => { const copy = document.querySelector('.eh-consent__copy')?.textContent || ''; const buttons = [...document.querySelectorAll('.eh-consent__button')]; return copy.includes('Чтобы нужное находилось быстрее') && copy.includes('Google Analytics') && copy.includes('без рекламного отслеживания') && !copy.includes('Яндекс') && buttons[0]?.textContent.trim() === 'Разрешить' && buttons[1]?.textContent.trim() === 'Google без cookies'; })()"));
