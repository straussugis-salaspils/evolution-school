  ...
  <div class="shell">
    ...
    <main>
      ...
      <section class="change-log" aria-labelledby="change-log-title">
        ...
        <ol>
          <li><time datetime="2026-08-31">31 августа 2026</time><div><strong>Сделали вступление в группу канонической конверсией</strong><p>Любое подтверждённое Telegram membership теперь сохраняется независимо от финальной CTA. Фоновая сверка восстанавливает пропущенные вступления недавних участников тестов, а Meta CompleteRegistration создаётся по факту group_joined без дублей.</p></div></li>
  ...
  <script>
    (function () {
      ...
      function renderFunnel(test, index) {
        ...
        return '<article class="funnel-card funnel-card--' + code + '" data-card="' + code + '"' + (code !== activeTest ? ' hidden' : '') + '><header class="funnel-head"><p class="funnel-kicker">Воронка ' + code.toUpperCase() + '</p><h2>' + escapeHtml(test.label) + '</h2><div class="headline-metrics"><div class="headline-metric"><strong>' + test.landing_visits + '</strong><span>сессий</span></div><div class="headline-metric"><strong>' + test.completed + '</strong><span>завершили тест</span></div><div class="headline-metric"><strong>' + test.group_joined + '</strong><span>вступили в группу</span></div></div></header><div class="journey">' + stepHtml + filterNote + note + '</div><div class="breakdowns">' + renderBreakdown("Источники", test.sources) + renderBreakdown("Кампании", test.campaigns) + '</div></article>';
      }
