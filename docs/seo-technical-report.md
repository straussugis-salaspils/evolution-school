...
Локальная SEO-версия технически готова к публикации: автоматический аудит проходит без ошибок, 42 канонические индексируемые страницы синхронизированы с sitemap, скрытые лендинги исключены из sitemap и имеют `noindex`, все локальные routes и assets существуют.
...
|---|---|---|
| Инвентаризация | ✅ | 56 HTML-файлов: 42 публичных индексируемых, 11 скрытых/noindex, 3 redirect alias |
...
| sitemap.xml | ✅ | Автоматически формируется из фактических canonical; 42 URL; без hidden, redirect и технических routes |
...
| Внутренние ссылки | ✅ | Битых ссылок нет; `/prodolzhit-put.html` и `/s-chem-priyti.html` получили естественные ссылки, а незаполненный блог исключён из индексации |
...
5. Для старых адресов индивидуального ретрита добавлены постоянные redirects в `vercel.json`; закрытый `/individualnyj-retrit/` сохранён самостоятельной посадочной страницей с self-canonical.
...
### Блог без опубликованных статей

`/blog.html` остаётся доступной страницей, но получила `noindex, follow` и исключена из sitemap до появления реальных статей. Внутренние ссылки на неё намеренно не добавлялись.
...
5. После появления реальных статей вернуть `/blog.html` в sitemap и заменить `noindex, follow` на `index, follow`.
