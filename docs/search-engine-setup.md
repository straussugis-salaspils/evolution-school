...
## Связь Search Console с GA4

После создания GA4 property и подтверждения Google Search Console откройте GA4 → Admin → Product links → Search Console links. Свяжите Web Data Stream `https://evolution.yourbalancerestored.com/` с соответствующим Search Console property. Для этого нужны права администратора в GA4 и подтверждённый доступ к Search Console; код сайта менять не требуется.
