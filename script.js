  ...
  const outcomeFor = () => {
    ...
    if (territory?.direct === 'retreat') {
      ...
      return {
        ...
        context,
        reason: 'Проблемы идут одна за другой и затрагивают не один эпизод. Ретрит можно рассмотреть как основной формат смены направления; Off-Switch остаётся точечной альтернативой для повторяющегося механизма.',
        primary: 'retreat',
        secondary: 'offswitch',
      };
    }
    ...
    if (state.diagnostic === 'globalResource') {
      return {
        ...
        context,
        reason: 'Нехватка сил проявляется не только в выбранной теме, но и в обычной жизни. Поэтому первым шагом становится практика восстановления; Off-Switch остаётся вторым вариантом, если внутри темы есть конкретная реакция.',
        primary: 'reiki',
        secondary: 'offswitch',
      };
    }
    ...
    if (state.diagnostic === 'localizedTrigger') {
      ...
      return {
        ...
        context,
        reason: isFinal
          ? 'Цель и действие уже ясны, а проблема остаётся в одном конкретном моменте. Квантовая активация подходит как завершающий сдвиг; Off-Switch — как практический формат для повторяющейся реакции.'
          : widerOption
    ...
    if (state.diagnostic === 'finalPoint') {
      return {
        ...
        context,
        reason: 'Главные решения уже приняты, и осталась одна конкретная точка. Квантовая активация становится основным форматом, а Off-Switch — альтернативой, если точку удерживает повторяющаяся реакция.',
        primary: 'activation',
        secondary: 'offswitch',
      };
    }
